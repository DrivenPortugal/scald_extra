  ...
  init: function (editor) {
    ...
    editor.addCommand('atomEdit', {
      exec: function (editor) {
        ...
        // Generate atom edit url
        var href = Drupal.settings.basePath + Drupal.settings.pathPrefix + 'atom/' + widget.data.sid + '/edit/nojs';

        // Send a message to parent so iframe modal is shown
        window.parent.postMessage({type:'show', href: href, sid: widget.data.sid}, location);

        // Add listener to detect if a close message is being sent
